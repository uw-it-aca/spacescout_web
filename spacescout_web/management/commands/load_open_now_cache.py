...
class Command(BaseCommand):
    help = ("Fills a cache with the json and image data for open spaces "
            "in all campuses")

    def handle(self, *args, **kwargs):
        if not hasattr(settings, 'SS_LOCATIONS'):
            raise("Error running load_open_now_cache - you need "
                  "to define your locations")

        for location in settings.SS_LOCATIONS:
            image_ids = []
            spaces = fetch_open_now_for_campus(location,
                                               use_cache=False,
                                               fill_cache=True)
